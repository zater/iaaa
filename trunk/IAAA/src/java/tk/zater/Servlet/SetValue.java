...
public class SetValue extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            PlanTable pt = (PlanTable) sess.get(PlanTable.class, Integer.parseInt(id));
            double newscore = (pt.getDownload() * pt.getScore()+Integer.parseInt(score)) / (pt.getDownload() + 1);
            pt.setScore(newscore);
            pt.setDownload(pt.getDownload() + 1);
            sess.update(pt);
            tx.commit();
        }
    }
